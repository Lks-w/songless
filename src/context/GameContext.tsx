  ...
  useEffect(() => {
    const initGame = async () => {
      try {
        ...
        if (hints.length === 0) {
          setHints([
            { type: 'views', revealed: false, content: `${(song.viewCount / 1000000).toFixed(1)}M views` },
            { type: 'likes', revealed: false, content: `${(song.likeCount / 1000).toFixed(1)}K likes` },
            { type: 'year', revealed: false, content: song.publishedAt.toString() },
  ...
  const revealHint = () => {
    ...
    if (nextHintIndex !== -1) {
      ...
      const hintMessages: Record<string, string> = {
        views: `Vistas: ${hintContent}`,
        likes: `Likes: ${hintContent}`,
        year: `Año de publicación: ${hintContent}`,
